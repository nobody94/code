...
function saveLayers(ref) {
	...
	var directory = prefs.filePath + "/" + ref.name;
	
	var outFolder = new Folder(directory);
	if (!outFolder.exists) {
		outFolder.create();
	}else {
		directory = directory + '_Images';
		var outFolder = new Folder(directory);
		if (!outFolder.exists) {
			outFolder.create();
		}
		...
		saveImage(directory, layer.name);
...
function saveImage(directory, layerName) {
    ...
    if(prefs.fileType=="PNG" && prefs.fileQuality=="8") {
        ...
        //SavePNG24(handle);
		SavePNGForWeb(handle);
    } else {
        // SaveJPEG(handle); 
        SaveJPEGForWeb(handle); 
    }
}
...
function SaveJPEGForWeb(saveFile){
	var jpgOpts = new ExportOptionsSaveForWeb; 
	jpgOpts.format = SaveDocumentType.JPEG;
	// jpgOpts.PNG8 = false; 
	// jpgOpts.transparency = false; 
	jpgOpts.interlaced = false; 
	jpgOpts.quality = 100;
	activeDocument.exportDocument(new File(saveFile+'.jpg'),ExportType.SAVEFORWEB,jpgOpts); 
}

function SavePNGForWeb(saveFile){
